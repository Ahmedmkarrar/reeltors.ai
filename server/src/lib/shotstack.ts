...
export interface GenerateMixedMediaOptions extends Omit<GenerateVideoOptions, 'images'> {
...
function getTemplateConfig(templateKey?: string): TemplateConfig {
  switch (templateKey) {
    ...
    case 'LUXURY_MANSION':
      return {
        accentColor:  PALETTE.GOLD, overlayColor: PALETTE.OVERLAY_DARK,
        priceFont:    "'Playfair Display', Georgia, serif",
        detailFont:   "'Montserrat', Arial, sans-serif",
        introLength:  5.5, clipLength: 4.5,
        mediaEffects: ['zoomIn', 'zoomOut', 'zoomIn', 'zoomOut', 'zoomIn', 'zoomOut'],
        clipTransIn:  'fade', clipTransOut: 'fade', textTransIn: 'slideUp', audioFadeAt: 15,
      };
    case 'CINEMATIC':
      return {
        accentColor:  PALETTE.GOLD, overlayColor: PALETTE.OVERLAY_DARK,
        priceFont:    "'Playfair Display', Georgia, serif",
        detailFont:   "'Montserrat', Arial, sans-serif",
        introLength:  5, clipLength: 4,
        mediaEffects: ['zoomIn', 'slideLeft', 'zoomOut', 'slideRight', 'zoomIn', 'slideLeft'],
        clipTransIn:  'fade', clipTransOut: 'fade', textTransIn: 'slideUp', audioFadeAt: 15,
      };
    case 'MODERN_MINIMAL':
      return {
        accentColor:  PALETTE.SILVER, overlayColor: PALETTE.OVERLAY_COOL,
        priceFont:    "'Montserrat', Arial, sans-serif",
        detailFont:   "'Montserrat', Arial, sans-serif",
        introLength:  4.5, clipLength: 3.5,
        mediaEffects: ['slideLeft', 'slideRight', 'slideLeft', 'slideRight', 'slideLeft', 'slideRight'],
        clipTransIn:  'fade', clipTransOut: 'fade', textTransIn: 'slideLeft', audioFadeAt: 15,
      };
    case 'TIKTOK_FAST':
      return {
        accentColor:  PALETTE.TIKTOK_RED, overlayColor: PALETTE.OVERLAY_DEEP,
        priceFont:    "'Montserrat', Arial, sans-serif",
        detailFont:   "'Montserrat', Arial, sans-serif",
        introLength:  2.5, clipLength: 2,
        mediaEffects: ['slideLeft', 'slideRight', 'slideLeft', 'slideRight', 'slideLeft', 'slideRight'],
        clipTransIn:  'wipeLeft', clipTransOut: 'wipeLeft', textTransIn: 'slideLeft', audioFadeAt: 12,
      };
    case 'STORY':
      return {
        accentColor:  PALETTE.STORY_BLUE, overlayColor: PALETTE.OVERLAY_DARK,
        priceFont:    "'Playfair Display', Georgia, serif",
        detailFont:   "'Montserrat', Arial, sans-serif",
        introLength:  4, clipLength: 3.5,
...
function buildTextClips(opts: {
  ...
  if (address || price) {
    clips.push({
      ...
      position: 'bottomLeft', offset: { x: 0.04, y: 0.07 },
  ...
  if (agentName) {
    clips.push({
      ...
      start: 0, length: totalDuration,
      position: 'topRight', offset: { x: -0.04, y: -0.04 },
    });
  }
  ...
  if (logoUrl) {
    clips.push({
      ...
      start:    0, length: totalDuration,
      position: 'topLeft', offset: { x: 0.04, y: -0.04 },
      opacity:  0.95, scale: 0.07,
    });
  }
...
function buildTimeline(
  ...
  const textClips     = buildTextClips({
    totalDuration, address: opts.address, price: opts.price,
    agentName: opts.agentName, logoUrl: opts.logoUrl, config, format: opts.format,
  });

  const audioDuration = Math.min(totalDuration, config.audioFadeAt);
  const tracks: { clips: ShotstackClip[] }[] = [];
  if (textClips.length > 0) tracks.push({ clips: textClips });
  if (opts.audioUrl) {
    tracks.push({ clips: [{
      asset: { type: 'audio', src: opts.audioUrl, trim: 0, volume: 0.45, effect: 'fadeOut' },
      start: 0, length: audioDuration,
    }] });
  }
...
function getShotstackEnv(env?: ShotstackEnv): ShotstackEnv {
  return (env ?? process.env.SHOTSTACK_ENV ?? 'stage') as ShotstackEnv;
}
...
export async function createRender(opts: {
  ...
  if (process.env.MOCK_AI === 'true') return { id: `mock_${Date.now()}`, status: 'queued' };
  ...
  const env  = getShotstackEnv(opts.env);
  const body: Record<string, unknown> = {
    timeline: opts.timeline,
    output:   { format: 'mp4', size: FORMAT_SIZES[opts.format ?? 'vertical'] },
  };
...
export async function generateMixedMediaVideo(opts: GenerateMixedMediaOptions): Promise<ShotstackRenderResponse> {
  ...
  return createRender({ timeline, format: opts.format ?? 'vertical', callbackUrl: opts.callbackUrl, apiKey: opts.apiKey, env: opts.env });
}

export async function getRenderStatus(renderId: string, apiKey?: string, env?: ShotstackEnv): Promise<ShotstackRenderResponse> {
  if (process.env.MOCK_AI === 'true') return { id: renderId, status: 'done', url: MOCK_VIDEO_URL };

  const key = (apiKey ?? process.env.SHOTSTACK_API_KEY ?? '').trim();
  if (!key) throw new ShotstackError('SHOTSTACK_API_KEY is not set');

  const res = await fetch(`${BASE_URL}/${getShotstackEnv(env)}/render/${renderId}`, {
  ...
  const json = await res.json() as {
    success:  boolean;
    response: { id: string; status: ShotstackStatus; url?: string; thumbnail?: string | null; error?: string };
  };
  const r = json.response;
  return { id: r.id, status: r.status, url: r.url, thumbnail: r.thumbnail ?? undefined, error: r.error };
}

      ...
      <div className="relative z-10 max-w-2xl mx-auto p-6 md:p-8 space-y-6">
        ...
        <section className="rounded-xl border border-[#C9A84C]/25 bg-white/80 overflow-hidden backdrop-blur-sm shadow-[0_2px_12px_rgba(201,168,76,0.08)]">
          ...
          <form onSubmit={handleSave}>
            ...
            <div className="px-6 py-4 bg-[#F5F2EC]/50 border-t border-[#F0EDE6] flex items-center justify-between">
              <p className="text-[11px] text-[#B8B4AE]">Changes are saved immediately.</p>
              <button
                type="submit"
                disabled={saving || nameUnchanged}
                className="btn-gold inline-flex items-center gap-2 h-9 px-5 rounded-lg text-[13px] font-bold transition-all duration-150
        ...
        {profile.subscription_status === 'canceled' && (
          <div className="rounded-xl border border-[#E2DED6] bg-[#FAF9F7] px-5 py-3">
            <p className="text-[13px] text-[#6B6760]">Your subscription has been canceled. Upgrade below to regain access.</p>
          </div>
        )}
        ...
        {/* subscription */}
        <section id="upgrade" className="rounded-xl border border-[#C9A84C]/25 bg-white/80 overflow-hidden backdrop-blur-sm shadow-[0_2px_12px_rgba(201,168,76,0.08)] scroll-mt-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {(['starter', 'growth', 'pro'] as const).map((key) => {
                  ...
                  return (
                    <div
                      key={key}
                      className="relative rounded-xl border p-4 flex flex-col transition-all duration-150 hover:shadow-[0_4px_12px_rgba(201,168,76,0.15)]"
                      style={{
                        borderColor: isPopular ? 'rgba(201,168,76,0.45)' : 'rgba(201,168,76,0.15)',
                        background:  isPopular ? 'rgba(240,180,41,0.03)' : 'rgba(255,255,255,0.6)',
                      }}
                    >
                      {isPopular && (
                        <div className="absolute -top-3 left-1/2 -translate-x-1/2 text-[#1A1714] text-[9px] font-bold px-3 py-0.5 rounded-full whitespace-nowrap tracking-wide shadow-[0_2px_8px_rgba(240,180,41,0.3)] btn-gold">
                          MOST POPULAR
                        </div>
                      )}
                      ...
                      <div className="flex items-baseline gap-0.5 mb-3">
                        <span
                          ...
                          style={{ color: isPopular ? '#C9930A' : '#6B6760' }}
                        >
                          ${p.price}
                        </span>
                      ...
                      <button
                        ...
                        className={[
                          ...
                          isPopular
                            ? 'btn-gold'
                            : 'border border-[#C9A84C]/30 text-[#6B6760] bg-white/60 hover:border-[#C9A84C]/50 hover:text-[#1A1714]',

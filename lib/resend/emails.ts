...
export async function sendWelcomeEmail(email: string, name: string) {
  return getResend().emails.send({
    ...
    html: `
      <!DOCTYPE html>
      <html>
      <body style="background:#080808;color:#F0F0EB;font-family:Inter,sans-serif;padding:40px 20px;margin:0;">
...
export async function sendFirstVideoEmail(email: string, name: string, videoUrl: string) {
  return getResend().emails.send({
    from: FROM(),
    to: email,
    subject: 'Your first listing video is ready 🎬',
    html: `
      <!DOCTYPE html>
      <html>
      <body style="background:#080808;color:#F0F0EB;font-family:Inter,sans-serif;padding:40px 20px;margin:0;">
...
export async function sendUpgradeNudgeEmail(email: string, name: string) {
  return getResend().emails.send({
    from: FROM(),
    to: email,
    subject: "You've used your free video for this month",
    html: `
      <!DOCTYPE html>
      <html>
      <body style="background:#080808;color:#F0F0EB;font-family:Inter,sans-serif;padding:40px 20px;margin:0;">

...
const stats = [
  { value: '60 sec',  label: 'avg creation time' },
  { value: '3',       label: 'platforms per video' },
  { value: '403%',    label: 'more buyer inquiries with video*' },
  { value: '$19',     label: 'to get started' },
          ...
          {stats.map((s, i) => (
            <FadeIn key={s.label} delay={i * 70} direction="none">
              <div className={`flex flex-col items-center text-center py-4 px-3 ${i < stats.length - 1 ? 'md:border-r border-[#1E1C18]' : ''} ${i === 1 ? 'border-b md:border-b-0 border-[#1E1C18]' : ''}`}>
                <div
                  className="font-syne font-extrabold text-[#F0B429] leading-none mb-2"

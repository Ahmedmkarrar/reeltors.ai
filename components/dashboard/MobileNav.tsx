...
export function MobileNav() {
  ...
  return (
    <nav
      className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-[#EBEBEB]"
        ...
        {NAV_ITEMS.map(({ href, label, icon }) => {
          ...
          return (
            ...
            >
              {/* Active dot */}
              {isActive && (
                <span className="absolute top-0 left-1/2 -translate-x-1/2 w-4 h-[2px] bg-[#1A1714] rounded-full" />
              )}
              <span className={isActive ? 'text-[#1A1714]' : 'text-[#ADADAD]'}>
                {icon}
              </span>
              <span className={`text-[9px] font-medium tracking-wide ${isActive ? 'text-[#1A1714]' : 'text-[#ADADAD]'}`}>
                {label}
              </span>
            </Link>
          );
